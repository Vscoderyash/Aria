...
export default function OwnerDashboard() {
  ...
  if (status === "loading") return <p style={{ color: 'white', textAlign: 'center', marginTop: '50px' }}>Loading Aria Systems...</p>;
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#050505', color: 'white' }}>
        <div style={{ padding: '40px', border: '1px solid #FFD700', borderRadius: '12px', textAlign: 'center', boxShadow: '0 0 20px rgba(255, 215, 0, 0.2)' }}>
          ...
          <button onClick={() => signIn()} style={{padding: '12px 24px', background: '#FFD700', color: 'black', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold'}}>
            Sign in as Owner
          </button>
        </div>
      </div>
  ...
  const pushToGitHub = async () => {
    ...
    if (data.success) {
      ...
      setFilePath(""); setCode(""); setMessage("");
    } else {
  ...
  return (
    <div style={{ padding: '40px', backgroundColor: '#050505', color: 'white', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #333', paddingBottom: '20px' }}>
        ...
        <button onClick={signOut} style={{background: 'transparent', color: '#FFD700', border: '1px solid #FFD700', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer'}}>Sign Out</button>
      </div>
